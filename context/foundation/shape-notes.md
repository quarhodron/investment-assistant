...
checkpoint:
  ...
  gray_areas_resolved:
    ...
    - topic: "insight vs generic chat-app projects"
      decision: "investment-research-specific: prompts/analyses linked to companies, tickers, sectors; continue-analysis chains across prompts and models"
    - topic: "primary persona scope"
      decision: "retail amateur investors, multi-tenant from v1 (each user isolated)"
    - topic: "auth method"
      decision: "email + password with email verification; password reset via email"
    - topic: "role model"
      decision: "flat — every user equal; no admin role, no shared workspaces"
    - topic: "onboarding state"
      decision: "empty state on first login with guided empty-state CTAs"
    - topic: "gated route behavior"
      decision: "redirect to /auth/signin with ?next=<original-path>; preserve intent"
    - topic: "v1 scope"
      decision: "full idea-notes spec — all 8 pages and all functional requirements (5.1–5.12). User accepts the scope and commits to the after-hours work."
    - topic: "MVP timeline"
...
- **Authentication.** Email + password. Email verification required on registration. Password reset via email.
- **Roles.** Flat. Every authenticated user can do everything within their own data. No admin role for v1, no shared workspaces, no read-only viewers.
- **Onboarding.** Empty state on first login. New user lands on the dashboard with no prompts, no analyses, no watchlist; guided empty-state CTAs show how to add the first prompt and run the first analysis.
- **Gated routes.** Unauthenticated users hitting a protected route are redirected to `/auth/signin` with the intent preserved (`?next=<original-path>`); after successful sign-in they are sent to the path they originally requested.
- **Isolation.** Every user's data is fully isolated. No user can read or modify another user's prompts, analyses, watchlist, API keys, or settings — neither through the UI nor through any API surface. Isolation applies across every entity in the data model.
...
- A user can browse the full history of their analyses and filter by date, by type (sector / company), and by associated company.
...
- A user never sees another user's prompts, analyses, watchlist, API keys, or settings — through the UI or any API surface.
- API keys are never returned to the client after they have been saved, and on disk they are protected against casual access (e.g., encrypted at rest).
...
- API cost / token usage of an analysis is visible to the user when the provider returns it, so the user knows roughly what an analysis cost.
...
- Estimated 1–3 weeks of after-hours work for v1.
- No hard deadline.
...
- FR-032: When the model returns sources, the analysis stores and displays them verbatim. Priority: must-have
  > Socrates: Verbatim per the guardrail in Phase 3. No counter-argument. Stands as written.
- FR-033: When the provider returns token usage and/or cost data, the analysis displays this information to the user. Priority: nice-to-have
  > Socrates: Counter-argument considered: "could be must-have given Phase 3 added 'API cost visible to user' as a guardrail." Resolution: kept nice-to-have at the FR level — Phase 3 guardrail is "if the provider returns it, show it"; the FR's must-have step is the provider returning the data, which we do not control. Display is best-effort.

## User Stories

### US-01: User runs first analysis on a sector
...
- **And when** they create a prompt, set an API key in Settings, and return to "New analysis"
- **And** they choose analysis type "Sector", their saved prompt, the default model, and enter "global, May 2026"
- **And** they click Run
...
- The model used for the run is the one shown in the dropdown at run time (default from Settings, overridable for the single run).
...
- API keys and any provider-issued credential are never returned to the client after they have been saved, and never appear in logs, error messages, or analytics events.
...
- **No investment recommendations or advice.** AI output is rendered as raw research material. The product never frames a result as a buy / sell / hold instruction. (Reinforces NFR #6.)
...
- **No market-data integration.** v1 does not connect to real-time price feeds, fundamentals databases, or news APIs. The user types tickers and topics by hand; the AI provides whatever its training and tools cover. Idea-notes 6.3 keeps integration as a future possibility — not v1.
...
- **No alerts or notifications.** No push, email, or in-app alerts for price moves, news, or analysis-ready events. v1 is pull-only — the user opens the app when they want to research.
- **No AI providers other than Anthropic and OpenAI.** v1 does not integrate Google Gemini, Mistral, locally-hosted models, or any other provider. Idea-notes 3.8 / 5.11 fixes this.
...
(Informational notes for the next chain step — `10x-tech-stack-selector`. Not part of the PRD schema.)

- **AI providers in v1.** Anthropic and OpenAI. The list of model variants is configurable so newly-released models can be added without redeploy.
- **API key storage.** Per-user. Encrypted at rest. Never returned to the client after save. Phase 3 guardrail.
- **Multi-tenant from v1.** Each user has fully isolated prompts, analyses, watchlist, settings. Implementation likely needs row-level security or equivalent.
